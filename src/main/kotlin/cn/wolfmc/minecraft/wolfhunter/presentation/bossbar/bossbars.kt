...
class NumberBossBar(
    ...
    init {
        subscribe<PlayerJoinEvent> {
            if(it.player.visibleCondition()) it.player.showBossBar(bar) else it.player.hideBossBar(bar)
        }.register()
        PluginScope.launch {
            while (true) {
                delay(1000)
    ...
    private fun updateProgress() {
        ...
        else if (now < min) bar.progress(0f)
        else bar.progress(now / max.toFloat())
    }
}
...
fun gameStarterBossBar(timeCounter: TimeCounter,max: Int) = NumberBossBar(
